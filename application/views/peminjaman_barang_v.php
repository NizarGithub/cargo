...
$(document).ready(function(){
	...
	$("#tambah_peminjaman_barang").click(function(){
		$("#tambah_peminjaman_barang").fadeOut('slow');
		$("#table_peminjaman_barang").fadeOut('slow');
		$(".cui").fadeOut('slow');
		$("#form_peminjaman_barang").fadeIn('slow');
		$("#tabel_total").fadeIn('slow');
	});

	$("#batal").click(function(){
		$("#tambah_peminjaman_barang").fadeIn('slow');
		$("#table_peminjaman_barang").fadeIn('slow');
		$("#form_peminjaman_barang").fadeOut('slow');
...
function hapus_peminjaman(id)
{
	...
	$('#popup_hapus').show();
		...
		$.ajax({
		url : '<?php echo base_url(); ?>peminjaman_barang_c/data_peminjaman_id',
		data : {id:id},
		type : "POST",
		dataType : "json",
		async : false,
...
function ubah_data_peminjaman(id)
{
	$("#tambah_peminjaman_barang").fadeOut('slow');
	$("#table_peminjaman_barang").fadeOut('slow');
	$("#form_peminjaman_barang").fadeIn('slow');
...
function ubah_detail(id){
	$.ajax({
		url : '<?php echo base_url(); ?>peminjaman_barang_c/data_peminjaman_detail_id',
		data : {id:id},
		type : "POST",
		dataType : "json",
		async : false,
...
function ajax_produk(id_form){
    ...
    $.ajax({
        url : '<?php echo base_url(); ?>peminjaman_barang_c/get_produk_popup',
        type : "POST",
        dataType : "json",
        data : {
            keyword : keyword,
...
function get_produk_detail(id, no_form){
	var id_produk = id;
    $.ajax({
		url : '<?php echo base_url(); ?>peminjaman_barang_c/get_produk_detail',
		data : {id_barang:id},
		type : "GET",
		dataType : "json",
		success : function(result){
...
function simpan_add_produk(){
	...
	if(nama_produk == ""){
		...
		$.ajax({
			url : '<?php echo base_url(); ?>peminjaman_barang_c/simpan',
			data : {
...
<div class="row" id="form_peminjaman_barang" style="display:none; ">
		...
		<div class="portlet light ">
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject font-green-sharp bold uppercase">Form peminjaman Barang</span>
				</div>
				<div class="actions">
			...
			<div class="portlet-body">
				<div class="row" style="padding-top: 15px; padding-bottom: 15px;">
					<div class="col-md-12">
						...
						<div class="col-md-3" style="display: none;" id="tetet">
							<label class="control-label"><b style="font-size:14px;">NO SPB1</b></label>
							<div class="input-group" style="width: 100%; ">
								<input type="text"  id="no_spb_a" name="no_spb_a" class="form-control">
							</div>
						</div>
					</div>
				</div>
				...
				<div class="row" style="padding-top: 35px; padding-bottom: 15px;">
					<div class="col-md-12">
						<div class="col-md-offset-2 col-md-10">
							...
							<button type="button" id="batal" class="btn red" onclick="window.location = '<?php echo base_url(); ?>peminjaman_barang_c'">Batal Dan Kembali</button>
						</div>
					</div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="col-md-2">
		<button id="tambah_peminjaman_barang" class="btn green" style="float: right;">
		Tambah Data peminjaman <i class="fa fa-plus"></i>
		</button>
	</div>
</div>
...
<div class="row" id="table_peminjaman_barang" style="display:block;">
	<div class="col-md-12">
		<!-- BEGIN EXAMPLE TABLE PORTLET-->
		<div class="portlet box green">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-edit"></i>Table peminjaman Barang
				</div>
				<div class="tools">
			...
			<div class="portlet-body">
				...
				<tr>
					...
					<td style="text-align:center; vertical-align: middle;">
						...
						<a class="btn default btn-xs red" id="hapus" onclick="hapus_peminjaman(<?php echo $value->id_peminjaman?>);"><i class="fa fa-trash-o"></i> Batal </a>
