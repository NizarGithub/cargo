...
$(document).ready(function(){
	...
	$("#tambah_bon_gudang_final").click(function(){
		$("#tambah_bon_gudang_final").fadeOut('slow');
		$("#table_bon_gudang_final").fadeOut('slow');
		$(".cui").fadeOut('slow');
		$("#form_bon_gudang_final").fadeIn('slow');
		$("#tabel_total").fadeIn('slow');
	});

	$("#batal").click(function(){
		$("#tambah_bon_gudang_final").fadeIn('slow');
		$("#table_bon_gudang_final").fadeIn('slow');
		$("#form_bon_gudang_final").fadeOut('slow');
...
function hapus_bon_gudang_final(id)
{
	...
	$('#popup_hapus').show();
		...
		$.ajax({
		url : '<?php echo base_url(); ?>bon_gudang_final_c/data_bon_gudang_final_id',
		data : {id:id},
		type : "POST",
		dataType : "json",
		async : false,
...
function ubah_data_bon_gudang_final(id)
{
	$("#tambah_bon_gudang_final").fadeOut('slow');
	$("#table_bon_gudang_final").fadeOut('slow');
	$("#form_bon_gudang_final").fadeIn('slow');
...
function ubah_detail(id){
	$.ajax({
		url : '<?php echo base_url(); ?>bon_gudang_final_c/data_bon_gudang_final_detail_id',
		data : {id:id},
		type : "POST",
		dataType : "json",
		async : false,
...
function ajax_produk(id_form){
    ...
    $.ajax({
        url : '<?php echo base_url(); ?>bon_gudang_final_c/get_produk_popup',
        type : "POST",
        dataType : "json",
        data : {
            keyword : keyword,
...
function get_produk_detail(id, no_form){
	var id_produk = id;
    $.ajax({
		url : '<?php echo base_url(); ?>bon_gudang_final_c/get_produk_detail',
		data : {id_barang:id},
		type : "GET",
		dataType : "json",
		success : function(result){
...
function tambah_data(){
	...
	var isi = 	'<tr id="tr_'+i+'">'+
					...
					'<td align="center" style="vertical-align:middle;">'+
						'<div class="controls">'+
							'<input style="font-size: 10px; text-align:center;" type="date" class="form-control form-control-inline input-medium date-picker" value="" name="tgl_pemakaian[]" id="tgl_pemakaian_'+i+'">'+
						'</div>'+
					'</td>'+
					'<td align="center" style="vertical-align:middle;">'+
						'<div class="controls">'+
...
function simpan_add_produk(){
	...
	if(nama_produk == ""){
		...
		$.ajax({
			url : '<?php echo base_url(); ?>bon_gudang_final_c/simpan',
			data : {
...
<input type="hidden" id="id_bon_gudang_final" name="id_bon_gudang_final">

<div class="row" id="form_bon_gudang_final" style="display:none; ">
		...
		<div class="portlet light ">
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject font-green-sharp bold uppercase">Form bon_gudang_final Barang</span>
				</div>
				<div class="actions">
				...
				<div class="row" style="padding-top: 35px; padding-bottom: 15px;">
					<div class="col-md-12">
						<div class="col-md-offset-2 col-md-10">
							...
							<button type="button" id="batal" class="btn red" onclick="window.location = '<?php echo base_url(); ?>bon_gudang_final_c'">Batal Dan Kembali</button>
						</div>
					</div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="col-md-2">
		<button id="tambah_bon_gudang_final" class="btn green" style="float: right;">
...
<div class="row" id="table_bon_gudang_final" style="display:block;">
	<div class="col-md-12">
		<!-- BEGIN EXAMPLE TABLE PORTLET-->
		<div class="portlet box green">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-edit"></i>Table Bon Gudang Final Barang
				</div>
				<div class="tools">
			...
			<div class="portlet-body">
				...
				<tr>
					...
					<td style="text-align:center; vertical-align: middle;">
						<!-- <a class="btn default btn-xs purple" id="ubah" onclick="ubah_data_bon_gudang_final(<?php echo $value->id_bon_gudang_final?>);"><i class="fa fa-edit"></i> Ubah </a> -->
						<a class="btn default btn-xs red" id="hapus" onclick="hapus_bon_gudang_final(<?php echo $value->id_bon_gudang_final?>);"><i class="fa fa-trash-o"></i> Batal </a>
						<a target="_blank" class="btn default btn-xs green" id="hapus" href="<?=base_url();?>bon_gudang_final_c/cetak/<?=$value->id_bon_gudang_final;?>" ><i class="fa fa-print"></i> Cetak </a>
					</td>
				</tr>
					<?php 
						}
